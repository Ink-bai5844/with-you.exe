...
func _sanitize_ai_event(event: Dictionary) -> Dictionary:
	return {
		...
		"distance_tiles": int(event.get("distance_tiles", 0)),
...
func _task_event_summary(event: Dictionary) -> String:
	...
	var item_id = str(event.get("item_id", event.get("resource", "")))
...
func _action_from_response(response: Dictionary, snapshot: Dictionary, allow_infer = false) -> Dictionary:
	...
	if type == "give_item" or type == "give" or type == "transfer_item":
		var item_id = str(action.get("item_id", action.get("item", action.get("resource", "")))).strip_edges()
		if not item_id.is_empty():
			return _with_interrupt_flag({
				"type": "give_item",
				"item_id": item_id,
